 ...
 */
class controller_session_referals extends rad_controller
{
    /**
        ...
        if($this->cookie($this->_cookieName)) {
            ...
            if( $item = $model->setState('cookie', $this->cookie($this->_cookieName))
                ...
                if($this->request('user_id')) {
                    if( $user = rad_instances::get('model_system_users')->getItem( (int)$this->request('user_id') ) ) {
    ...
    {
        $code = md5($userId);
        $itemType = model_session_referals::TYPE_INDEX;
        $itemId = 0;
        $item = new struct_referals(array(
                        'rrf_user_id'=>$userId,
