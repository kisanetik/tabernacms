...
class controller_system_managelang extends rad_controller
{
	...
	function getTranslations()
	{
		...
		$model = rad_instances::get('model_system_langvalues')->setState('lang_id', $lang_id)->setState('type', $this->request('type'))->setState('order by', 'lnv_code');
		if (!empty($code)){
	...
	function getNodes()
	{
		$items = rad_instances::get('model_system_lang')->getItems();
	...
	function showLand()
	{
		...
		if ((int)$this->request('node_id')){
			...
			$model = rad_instances::get('model_system_langvalues');
			$lang_model = rad_instances::get('model_system_lang')->setState('where', 'lng_id ='.(int)$this->request('node_id'))->getItems();
	...
	function delete()
	{
		if ((int)$this->request('lang_id')>0){
			$rows = rad_instances::get('model_system_lang')->deleteRow((int)$this->request('lang_id'));
			if ($rows>0){
				echo 'true';
				exit;
			}
