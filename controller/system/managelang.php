 ...
 * @version 0.3
 */
class controller_system_managelang extends rad_controller
{
    /**
     * Default language
     * @var string
    ...
    */
    ...
    function getTranslations()
    {
        ...
        $model = rad_instances::get('model_system_langvalues')
               ->setState('lang_id', $lang_id)
               ->setState('type', $this->request('type'))
               ->setState('order by', 'lnv_code');
        if ( !empty($code) ) {
    ...
    */
    ...
    function getNodes()
    {
        $items = rad_instances::get('model_system_lang')
                 -> getItems();
    ...
    */
    function location()
    {
        ...
        $url = $this->json_url = is_array($this->json_url)
                ? $this->json_url[$this->getContentLangID()]
                : $this->json_url;
    ...
    */
    function translations_download()
    {
        ...
        $url = is_array( $this->json_url )
                ? $this->json_url[ $lang_id ]
                : $this->json_url;
        ...
        foreach ($elements as $element) {
            $code   = rad_dbpdo::escapeString($element->lnv_code);
            $value  = rad_dbpdo::escapeString($element->lnv_value);
            $values[] = "('{$code}', '{$value}', {$lang_id})";
        }
        $query = '';
    ...
    */
    ...
    function showLand() {
        ...
        if ((int)$this->request('node_id')) {
            ...
            $model = rad_instances::get('model_system_langvalues');
            $lang_model = rad_instances::get('model_system_lang')
            ->setState('where', 'lng_id ='.(int)$this->request('node_id'))
            ->getItems();
            ...
            */
            ...
            //dvy//$this->setVar('lang', rad_instances::get('model_system_lang')->getItems() );
    ...
    */
    ...
    function deletelang()
    {
        if ((int)$this->request('lang_id')) {
            $rows = rad_instances::get('model_system_langvalues')->deleteRow((int)$this->request('lang_id'));
            if ($rows) {
                echo 'RADLangList.message("'.addslashes( $this->lang('deletedrows.lang.message') ).': '.$rows.'");';
    ...
    function delete()
    {
        if ( (int)$this->request('lang_id') > 0 ) {
            $rows = rad_instances::get('model_system_lang')
                  ->deleteRow((int)$this->request('lang_id'));
            if ( $rows > 0 ) {
                echo 'true';
                exit;
            }
    ...
    */
    ...
    function addLangForm()
    {
        if ($this->request('lnv_lang')) {
            $this->setVar( 'langs', rad_instances::get('model_system_lang')->getItems() );
    ...
    */
    ...
    function changeLangs()
    {
        if ($this->request('lang_id')) {
            $langs = model_system_table::getInstance(RAD.'langvalues')->getItem($this->request('lang_id'));
            $langs->lnv_lang = $this->request('nl');
            $rows = model_system_table::getInstance(RAD.'langvalues')->updateItem($langs);
            if ($rows) {
    ...
    function uploadFile()
    {
        if ( array_key_exists('lng_image', $_FILES) ) {
            $lang_id = (int) $this->request('lng_id');
            if ( isset($_FILES)
                 and count($_FILES['lng_image'])
                 and (!$_FILES['lng_image']['error'])
                 and ($_FILES['lng_image']['size'] > 0 )
            ) {
                if ( count(glob(LANGORIGINALPATCH . '*' . $lang_id . '_lng*')) ) {
                    foreach (glob(LANGORIGINALPATCH.'*'.$lang_id.'_lng*') as $filename) {
                        unlink($filename);
                    }
                }
                $new_fn = strtolower(
                      $lang_id . '_lng'
                    . md5( now() . $this->getCurrentUser()->u_id . $_FILES['lng_image']['name'] )
                    . '.' . fileext( $_FILES['lng_image']['name'] )
                );
                $model = rad_instances::get('model_system_lang');
                $item = $model->getItem($lang_id);
                if ( move_uploaded_file($_FILES['lng_image']['tmp_name'], LANGORIGINALPATCH . $new_fn ) ) {
                    $item->lng_img = $new_fn;
    ...
    function save()
    {
        ...
        if ( $this->request('lng_id') > 0 ) {
            $item = $model->getItem( $this->request('lng_id') );
            ...
            $res = $model->insertItem($struct_lang) > 0 ? rad_dbpdo::lastInsertId() : 0;
        }
        echo $res;
        exit;
    }
