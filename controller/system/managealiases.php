 ...
 * @package RADCMS
 */
class controller_system_managealiases extends rad_controller
{
    ...
    {
        $model = rad_instances::get('model_system_aliases');
    ...
    function editRecord()
    {
        ...
        $model = new model_system_table(RAD.'templates');
    ...
    function addInclude()
    {
        if( ( $this->request('include_id')>0 ) and ( $this->request('position_id') ) and ($this->request('alias_id')) ) {
            $struct = new struct_includes_in_aliases($this->getAllRequest());
            $model = rad_instances::get('model_system_includes');
            ...
            if( $rows ) {
                ...
                $this->clearAliasCache(rad_instances::get('model_system_aliases')->getItem((int)$this->request('id')));
            } else {
    ...
    function addIncludeW()
    {
        if($this->request('hash') == $this->hash()) {
            if( ($this->request('include_id')) and ($this->request('position_id')) and ($this->request('order_sort')) ){
                $struct = new struct_includes_in_aliases();
                $aliasid = (int)$this->request('alias_id');
    
                $theme = trim($this->request('theme'));
                if( strlen($theme) and (is_dir(THEMESPATH.$theme)) ){
                    $table = new model_system_table(RAD.'themes');
                ...
                $model_aliases = rad_instances::get('model_system_aliases');
                ...
                $rows = $model->insertItem($struct);
                if($rows) {
                    $this->clearAliasCache(rad_instances::get('model_system_aliases')->getItem($struct->alias_id));
    ...
    {
        $table = new model_system_table(RAD.'templates');
    ...
    {
        $table = new model_system_table(RAD.'positions');
    ...
    private function _assignIncludesAndModules()
    {
        ...
        for($i=0;$i<$cnt_res;$i++) {
            if($theme_ex) {
                if( file_exists(THEMESPATH.$theme.DS.'micros'.DS.$modules_sort[$res[$i]['id_module']].DS.$res[$i]['inc_filename']) and is_file(THEMESPATH.$theme.DS.'micros'.DS.$modules_sort[$res[$i]['id_module']].DS.$res[$i]['inc_filename']) ) {
                    $res[$i]['is_theme'] = 1;
                } else {
                    $res[$i]['is_theme'] = 0;
                }
            }
			if(!file_exists(MICROSPATH.$modules_sort[$res[$i]['id_module']].DS.'set'.DS.$res[$i]['inc_filename'].'.xml')) {
			    if(defined('SYSTEMMICROSPATH')) {
			        if(!file_exists(SYSTEMMICROSPATH.$modules_sort[$res[$i]['id_module']].DS.'set'.DS.$res[$i]['inc_filename'].'.xml')) {
			            unset($res[$i]);
			        }
			    } else {
			        unset($res[$i]);
    ...
    function configInclude()
    {
        ...
        if($inc_id) {
			if($this->request('onlymain')) {
    			...
    			$this->setVar('inc_id',$inc_id);
                $item = rad_instances::get('model_system_includes')->getItem($inc_id);
			    $this->setVar('inc_item',$item);
                $params_real = new rad_paramsobject($item->params_hash);
                $item = rad_instances::get('model_system_aliases')->getIncludeById( $item->include_id );
			}
    ...
    function saveEditScript()
    {
        if($this->request('hash') == $this->hash()) {
            ...
            if($alias_id) {
                ...
                $this->clearAliasCache(rad_instances::get('model_system_aliases')->getItem($alias_id)->alias);
            } else {
                $this->securityHoleAlert(__FILE__,__LINE__,$this->getClassName());
            }
        } else {
    ...
    function saveDescription()
    {
        if($this->redirect('hash') == $this->hash()) {
            ...
            if( count($descriptiontxt) and $alias_id){
                $table = new model_system_table(RAD.'aliases_description');
                $rows = 0;
                foreach($descriptiontxt as $lng_id=>$description){
                    $description = trim($description);
                    if( strlen($description) ){
                        $item = new struct_aliases_description();
    ...
    function createTheme()
    {
        ...
        $alias_id = (int)$this->request('alias_id');
        if($alias_id) {
            $this->clearAliasCache(rad_instances::get('model_system_aliases')->getItem($alias_id));
            if( strlen($theme) and (is_dir(THEMESPATH.$theme)) ){
                $table = new model_system_table(RAD.'themes');
                $theme_item = new struct_themes();
