 ...
 * @version 0.2
 */
class controller_articles_articles extends rad_controller
{
    private $_pid = 19;
    /**
    ...
    function getItem()
    {
        ...
        if($art_id){
            $item = rad_instances::get('model_articles_articles')->getItem($art_id);
            $this->addBC( 'curr_item', $item);

            $this->setVar('currCategory', rad_instances::get('model_menus_tree')->getItem($item->art_treid));
            $this->_currArticleTreeItem = $item->art_treid;
            if(defined('ARTICLESORIGINALPATCH'))
                if(file_exists(ARTICLESORIGINALPATCH.$item->art_img) and is_file(ARTICLESORIGINALPATCH.$item->art_img))
                    $item->xy = getimagesize(ARTICLESORIGINALPATCH.$item->art_img);
    ...
    function assignMenu()
    {
        ...
        $categories = rad_instances::get('model_menus_tree')
    ...
    function assignOnMain ()
    {
        ...
        for ($i=0; $i < $count; $i++) {
            ...
            $models = rad_instances::get('model_articles_articles');
