 ...
 *
 */
class controller_catalog_managecattypes extends rad_controller
{
    private $_pid = '47';
    ...
    {
        rad_autoload_register(array('model_catalog_types', 'autoloadPlugins'));
    }
    ...
    function showEditNode()
    {
        if((int)$this->request('id')) {
            ...
            array_unshift($nodes, new struct_tree( array('tre_id'=>$this->_pid,'tre_name'=>$this->lang('rootnode.catalog.text')) ) );
    ...
    function addNewNode()
    {
        ...
        if($nodeId) {
            ...
            $rows = rad_instances::get('model_menus_tree')->insertItem($node);
            $id = rad_instances::get('model_menus_tree')->inserted_id();
            $parent_item = rad_instances::get('model_menus_tree')->getItem($nodeId);
            if($parent_item->tre_islast){
                $parent_item->tre_islast = '0';
                rad_instances::get('model_menus_tree')->updateItem($parent_item);
            }
            if($rows) {
    ...
    function addTypeFieldForm()
    {
        if((int)$this->request('id')) {
            ...
            $model_types = rad_instances::get('model_catalog_types');
            ...
            $ts_sh = array();
            if($this->request('vl_id')) {
                $model_sh = new model_system_table(RAD.'ct_showing');
    ...
    {
        if($type_id) {
            $model = new model_system_table(RAD.'ct_showing');
        ...
        if($type_id) {
            ...
            if($this->_showinlist) {
                if($this->request('CTshowing_showinlist_catalog_option')) {
                    $added_items[] = new struct_ct_showing( array(
                            'cts_vl_id'=>$type_id,
    ...
    {
        if($this->request('hash') == $this->hash()) {
            $item = new struct_cat_val_names($this->getAllRequest());
    ...
    function delTypeField()
    {
        if( ((int)$this->request('id')) and ((int)$this->request('tre_id')) ) {
        	$item = new struct_cat_val_names( array('vl_id'=>(int)$this->request('id')) );
        	$rows = $item->remove();
            if($rows) {
