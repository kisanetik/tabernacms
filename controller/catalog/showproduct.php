 ...
 *
 */
class controller_catalog_showproduct extends rad_controller
{
    ...
    function __construct()
    {
        ...
        if ($allowComments && $action == 'comment_add') {
            if ( empty($this->getCurrentUser()->u_id) ) {
                $captcha = new model_session_captcha('product');
        ...
        if ($pId) {
            ...
            if(!empty($product->cat_id)) {
                ...
                if($this->config('partners.3dbin.license')) {
                    rad_instances::get('model_catalog_3dimages')->assign3Dimage($product);
                }
                ...
                if($this->_showComments) {
                    $modelComments = rad_instances::get('model_resource_comments');
