 ...
 *
 */
class controller_catalog_filter extends rad_controller
{
    ...
    /**
     * Instance of category (struct_tree)
     * @var struct_tree
     */
    private $_cat = NULL;
	...
	function assignFilters()
	{
	    ...
	    if($this->request($this->_getcat)) {
	        $cat = rad_instances::get('model_menus_tree')->getItem($this->request($this->_getcat));
	...
	private function _getCatsProducts()
	{
	    ...
	    if(!$result) {
	        $result = rad_instances::get('model_catalog_catalog')
	                    ->setState('active', 1)
	...
	protected function assignBrands()
	{
	    ...
	    if(!empty($cats)) {
	        ...
	        $this->setVar('brands',
	                        rad_instances::get('model_catalog_brands')
	    ...
	    if(!empty($this->_catIds)) {
	        ...
	        if(!empty($valValues)) {
	            ...
	            foreach($valValues as $valId=>$valVal) {
	                $valNames[$valId] = new struct_cat_val_names(array('vl_id'=>(int)$valId));
	...
	{
	    if(!empty($this->_catIds)) {
	        $items = rad_instances::get('model_catalog_types')
	...
	protected function assignFilterPrice()
	{
	    ...
	    if(!empty($this->_catIds)) {
	        $minmax = rad_instances::get('model_catalog_catalog')
