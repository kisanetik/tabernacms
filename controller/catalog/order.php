 ...
 *
 */
class controller_catalog_order extends rad_controller
{
    ...
    function order()
    {
    	$item = new struct_orders();
    	$item->order_userid = ($this->getCurrentUser())?$this->getCurrentUser()->u_id:0;
        if($this->_showCaptcha or $this->_OrderScheme[self::ORDER_REG]) {
    		$captcha = new model_session_captcha(SITE_ALIAS);
    	...
    	//calc the order summ
    	$model_bin = rad_instances::get('model_catalog_bin');
        $ct_showing = ($this->getParamsObject())?$this->getParamsObject()->ct_showing:NULL;
        ...
        if($this->_showDelivery and $this->request('delivery')) {
            $delivery = new struct_delivery(array('rdl_id'=>(int)$this->request('delivery')));
            $delivery->load();
            $totalCostsWithoutDelivery = $total_costs;
            $total_costs += model_catalog_currcalc::calcCours( $delivery->rdl_cost, $delivery->rdl_currency );
            $item->delivery = $delivery;
        }
        $item->order_summ = $total_costs;
        $item->order_currency = model_catalog_currcalc::$_curcours->cur_ind;
    	$item->order_curid = model_catalog_currcalc::$_curcours->cur_id;
    	...
    	$model = rad_instances::get('model_catalog_order');
    	if($rows = $model->insertItem($item)) {
    	    $item->order_id = $rows;
    	    $item->order_num .= $item->order_id;
    	    $item->save();
    	    rad_instances::get('model_catalog_bin')->clearItemsCart();
    	    /*assign to the referals*/
    	    if($this->config('referals.on') and class_exists('struct_referals_orders')) {
    	        ...
    	        if($this->cookie($this->config('referals.cookieName')) or !empty($item->order_userid)) {
    	            if(!empty($item->order_userid)) {
    	                $refUser = rad_instances::get('model_session_referals')->getUserPartner($item->order_userid);
    	            }
    	            if(!empty($refUser->u_id)) {
    	                $referalId = $refUser->rru_referal_id;
    	            } elseif($referal = rad_instances::get('model_session_referals')
        	                                      ->setState('cookie', $this->cookie($this->config('referals.cookieName')))
                                                  ->getItem()) {
    	                $referalId = $referal->rrf_id;
    	            }
    	            if(!empty($referalId)) {
        	            $percent = rad_instances::get('model_session_referals')->getParntnerPercent($referalId);
        	            $orderSum = (isset($totalCostsWithoutDelivery)?$totalCostsWithoutDelivery:$item->order_summ);
          	            $refOfder = new struct_referals_orders(
          	                            array(
          	            ...
          	            rad_instances::get('model_session_referals')->insertOrder($refOfder);
    	            }
    	        }
    	    }
    	}
    ...
    function showCapcha()
    {
        $image = controller_session_registersimply::getSIObj();
        $image->show();
    }
    ...
    private function _sendMail(struct_orders $order, $type)
    {
        switch($type) {
        	case 'order_new':
