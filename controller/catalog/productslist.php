 ...
 *
 */
class controller_catalog_productslist extends rad_controller
{
    private $_pid = 38;
    ...
    function assignProducts()
    {
        ...
        if(!$special_offer) {
            ...
            $tree_item = rad_instances::get('model_menus_tree')->getItem($cat);
        ...
        if($this->request('filter')) {
            ...
            if((float)$this->request('costfrom')) {
                ...
                $model->setState('currency', model_catalog_currcalc::getDefaultCurrencyId());
            }
        ...
        switch($this->request('o')) {
            case 'name':
                ...
                break;
            case 'cost':
                $order.='c.cat_cost*(cr.cur_cost/'.model_catalog_currcalc::currCours().')';
                break;
            default:
                $order.='c.cat_name';
                break;
        ...
        foreach($products as $pkey=>$product) {
            if(count($product->type_vl_link)) {
                foreach($product->type_vl_link as $tvlkey => $tvl) {
                    if($product->type_vl_link[$tvlkey]->vl_measurement_id) {
                        ...
                        $mes = new struct_measurement( array('ms_id'=>$mes_id) );
                        $mes->load();
            ...
            if($this->config('partners.3dbin.license')) {
                rad_instances::get('model_catalog_3dimages')->assign3Dimages($products);
            }
        }
