 ...
 *
 */
class controller_catalog_shownews extends rad_controller
{
	private $_pid = 16;
...
*/
	...
	function getLastItems()
	{
		...
		$category = rad_instances::get('model_menus_tree')->getItem($cat);
		if (!$this->_isLeftMenu)
	...
	function getOneNews()
	{
		$nid = (int)$this->escapeString($this->request('nid'));
		if ($nid) {
			$model = rad_instances::get('model_catalog_news');
			$model->setState('nw_folk', $this->_isfolknews);
			$model->setState('nw_active', 1);
			...
			if (!(int)$item->nw_id or ($this->request('title') and $this->request('title') != $item->nw_title)) {
				...
				$cur_category = rad_instances::get('model_menus_tree')->getItem($item->nw_tre_id);
				if (!$this->_isLeftMenu) {
	...
	function getOrganizationNews()
	{
		...
		if ($user_id) {
			...
			$model->setState('nw_folk', $this->_isfolknews);
			/* PAGING */
			$p = (int)$this->escapeString($this->request('p'));
			$page = ($p) ? $p : 0;
			$limit = ($page * $this->_itemsperpage) . ',' . $this->_itemsperpage;
