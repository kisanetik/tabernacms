 ...
 *
 */
class controller_catalog_manageorders extends rad_controller
{
    /**
    ...
    {
    	$model = rad_instances::get('model_catalog_order')->setState('lang_id', $this->getContentLangID());
    ...
    function showEdit()
    {
    	...
    	if($order_id) {
    		...
    		if($order->order_delivery) {
    		    $order->delivery = new struct_delivery( array('rdl_id'=>$order->order_delivery) );
    		    $order->delivery->load();
    		}
    		...
    		if(count($order->order_positions)){
    			foreach($order->order_positions as $id){
    				...
    				$total_cost += ($id->orp_count * model_catalog_currcalc::calcCours( $id->orp_cost, $id->orp_curid ) );
    			}//foreach
    		}//if count
    		$this->setVar('total_cost',$total_cost);
    		$this->setVar('total_count',$total_count);
    		$this->setVar('currency', model_catalog_currcalc::$_curcours );
    		$this->assignTypes();
    	} else {
    		$this->securityHoleAlert(__FILE__,__LINE__,$this->getClassName());
    	}
        ...
        if($this->request('hash') == $this->hash()) {
        	...
        	if( $orderId ) {
        		...
        		if($this->config('referals.on') and class_exists('struct_referals_orders')) {
                    rad_instances::get('model_session_referals')->deleteOrderEvent($orderId);
        		}
    ...
    function addProducts()
    {
        if($this->request('hash') == $this->hash()) {
            ...
            if($orderId && !empty($newProducts)) {
                rad_instances::get('model_catalog_order')->addToPositions($orderId, $newProducts);
                rad_instances::get('model_session_referals')->recalcOrderSum($orderId);
    ...
    function deletePosition()
    {
        if($this->request('hash') == $this->hash()) {
            ...
            if($orderId > 0  and $catId > 0) {
                $r = rad_instances::get('model_catalog_order')->deletePositionFromOrder($orderId, $catId);
                rad_instances::get('model_session_referals')->recalcOrderSum($orderId);
                echo 'RADOrders.refresh();';
