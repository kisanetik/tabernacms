...
function smarty_function_currency($params, $smarty)
{
    ...
    $cost = empty($params['noconvert'])
        ? model_corecatalog_currcalc::calcCours($params['cost'], (int)$params['curid'])
        : $params['cost'];
    return empty($params['noformat'])
        ? model_corecatalog_currcalc::formatCost($cost, empty($params['noconvert']) ? null : (int)$params['curid'])
        : $cost;
}