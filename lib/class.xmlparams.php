...
final class rad_xmlparams
{
	...
	{
		$result = is_file( MICROSPATH.$module.DS.'set'.DS.$fn.'.xml' );
		if(!$result and defined('SYSTEMMICROSPATH')) {
		    $result = is_file(SYSTEMMICROSPATH.$module.DS.'set'.DS.$fn.'.xml');
		}
		return $result;
	}
    ...
    final private static function getXmlFile($module,$fn, $theme=null)
    {
        if(self::checkXMLFor($module,$fn)) {
            if(defined('THEMESPATH') and is_file(THEMESPATH.$theme.DS.'micros'.DS.$module.DS.'set'.DS.$fn.'.xml')) {
                return file_get_contents( THEMESPATH.$theme.DS.'micros'.DS.$module.DS.'set'.DS.$fn.'.xml' );
            } elseif(is_file(MICROSPATH.$module.DS.'set'.DS.$fn.'.xml')) {
                return file_get_contents( MICROSPATH.$module.DS.'set'.DS.$fn.'.xml' );
            } elseif(defined('SYSTEMMICROSPATH') and is_file(SYSTEMMICROSPATH.$module.DS.'set'.DS.$fn.'.xml')) {
                return file_get_contents(SYSTEMMICROSPATH.$module.DS.'set'.DS.$fn.'.xml');
            } else {
