 ...
 * @pachage RADCMS
 */
class controller_coreresource_comments extends rad_controller
{
    /**
    ...
    function __construct()
    {
         ...
         $this->setVar('user', ($this->getCurrentUser())?$this->getCurrentUser():new struct_core_users());
     ...
     * @param struct_coreresource_comments $item - Комментарий.
     */
    function sendMail(struct_coreresource_comments $item)
    {
        ...
        //echo ($email_to); die();
        if($template->sct_id) {
            $mail = model_coremail_mail::getMailer();
            $mail->IsHTML(false);
    ...
    function assignItem()
    {
        if((int)$this->request('i')) {
            ...
            $item = rad_instances::get('model_coreresource_comments')
                    ->setState('active', 1)
