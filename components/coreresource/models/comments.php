 ...
 * @package RADCMS
 */
class model_coreresource_comments extends rad_model
{
    /**
     * Products type
     * @var string
    ...
    function getItem($id=null)
    {
        ...
        $item = (!empty($item)?new struct_coreresource_comments($item):null);
        return $item;
    }
    ...
    function getItems($limit = null)
    {
        ...
        if ($this->getState('select')!=='count(*)') {
            foreach ( $this->queryAll( $q->toString().$limit, $q->getValues() ) as $key) {
                $result[] = new struct_coreresource_comments($key);
                if ($this->getState('join.news') and (int)$key['nw_id']) {
                    $result[count($result)-1]->news = new struct_corearticles_news($key);
                }
                if ($this->getState('join.articles') and (int)$key['art_id']) {
                    $result[count($result)-1]->articles = new struct_corearticles_articles($key);
                }
                if ($this->getState('join.products') and (int)$key['cat_id']) {
                    $result[count($result)-1]->product = new struct_corecatalog_catalog($key);
                }
                if ($this->getState('join.orders') and (int)$key['order_id']) {
                    $result[count($result)-1]->order = new struct_corecatalog_orders($key);
                }
            }
        } else {
    ...
    /**
     * Insert one row
     * @param struct_coreresource_comments $item
     */
    function insertItem(struct_coreresource_comments $item)
    {
     ...
     * @param struct_coreresource_comments $item
     */
    function updateItem(struct_coreresource_comments $item)
    {
     ...
     * @param struct_coreresource_comments $item
     */
    function deleteItem(struct_coreresource_comments $item)
    {
