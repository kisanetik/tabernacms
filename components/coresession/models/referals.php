 ...
 *
 */
class model_coresession_referals extends rad_model
{
    /**
     ...
     * @example:
     * makeUrl(1, array(
     *     'type'=>model_coresession_referals::TYPE_PAGE,
     *     'item_id'=>34
     * ))
     */
    ...
    function getItem($id=null)
    {
        ...
        if($res) {
            return new struct_coresession_referals($res);
        } else {
            return null;
        }
    }
    ...
    function getItems()
    {
        ...
        if($res) {
            $result = array();
            foreach($res as $id) {
                $result[] = new struct_coresession_referals($id);
     ...
     * @param integer|struct_core_users $userId
     * @return struct_coresession_referals_users | NULL
     */
    function getUserPartner($userId)
    {
        if($userId instanceof struct_core_users) {
            $userId = $userId->u_id;
        }
        $userId = (int)$userId;
        $res = $this->query('SELECT * FROM '.RAD.'referals_users WHERE rru_user_id=:user_id', array('user_id'=>$userId));
        if(!empty($res['rru_id'])) {
            return new struct_coresession_referals_users($res);
        }
        return NULL;
    }
     ...
     * @param integer | struct_core_users $parnerId
     * @state rrf_user_id integer | struct_core_users $parnerId
     * @return float
     */
    function getParntnerPercent($partnerId=NULL)
    {
        ...
        if($partnerId instanceof struct_core_users) {
...
//         die($q->toString().$limit.print_h($q->getValues(),true));
     ...
     * @param struct_coresession_referals_orders $item
     * @return struct_coresession_referals_orders
     */
    function insertOrder(struct_coresession_referals_orders $item)
    {
     ...
     * @param struct_coresession_referals_users $item
     * @return struct_coresession_referals_users
     */
    function insertUser(struct_coresession_referals_users $item)
    {
    ...
    function insertItem(struct_coresession_referals $item)
    {
