var RADOrderForm = function(initObject) {
    
    this.docRoot = SITE_URL;

    this.elementIdFio = 'fio';
    this.elementIdEmail = 'email';
    this.elementIdPhone = 'contact_phone';
    ...
    // List of required
    this.RequiredFields = ['fio', 'email', 'contact_phone'];
    if($('#captcha_text').length) {
        this.RequiredFields[this.RequiredFields.length] = 'captcha_text';
    }
...
var RADOrderForm = new RADOrderForm();
if($('#delivery_select').length) {
    $('#delivery_select').change(function(){
        $('.delivery-description').hide();
        $('#del_desc_'+$('#delivery_select :selected').val()).show('fast');
        if($('#delivery_select :selected').attr('cost').length && $('#delivery_select :selected').attr('cost')!=0) {
            $('#total-with-delivery').html( parseInt($('#total_costs').html())+parseInt($('#delivery_select :selected').attr('cost')) );
            $('#delivery-costs-block').show();
            $('#hid_delivery').val($('#delivery_select :selected').val());
        } else {
            $('#delivery-costs-block').hide();
            $('#hid_delivery').val(0);
        }
    });
}