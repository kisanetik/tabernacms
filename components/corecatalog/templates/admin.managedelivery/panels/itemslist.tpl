    ...
    <tr style="height:42px;">
        <td>
            <a id="active_del_link_{$item->rdl_id}_1" href="javascript:RADDelivery.setActive(0,{$item->rdl_id});" {if !$item->rdl_active} style="display:none;"{/if}>
                <img id="img_active_cat_{$item->rdl_id}" src="{url type="image" module="core" preset="original" file="actions/activeround.gif"}" border="0" alt="{lang code='-turnoff' ucf=true|replace:'"':'&quot;'}" title="{lang code='-turnoff' ucf=true|replace:'"':'&quot;'}" />
            </a>
            <a id="active_del_link_{$item->rdl_id}_0" href="javascript:RADDelivery.setActive(1,{$item->rdl_id});" {if $item->rdl_active} style="display:none;"{/if}>
                <img id="img_active_cat_{$item->rdl_id}" src="{url type="image" module="core" preset="original" file="actions/notactiveround.gif"}" border="0" alt="{lang code='-turnon' ucf=true|replace:'"':'&quot;'}" title="{lang code='-turnon' ucf=true|replace:'"':'&quot;'}" />
            </a>
        </td>
        <td>{$item->rdl_name}</td>
