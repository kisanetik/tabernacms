                ...
                <div class="bin_goods_list">
                    {foreach $items as $item}
                        <div class="bin_field_good">
                            <div class="right_side">{$item->cat_cost*$item->cat_count|string_format:"%.2f"} {currency get="shortname"}</div>
                            {$item->cat_name} <span>({$item->cat_cost|string_format:"%.2f"} &times; {$item->cat_count|string_format:"%.0f"})</span>
                        </div>
                    {/foreach}
                    <div class="total_costs">{lang code="-total" ucf=true} <span>{$total_costs|string_format:"%.2f"} {if $total_costs}{currency get="shortname"}{/if}</span></div>
                </div>
            {/if}
...
{else}
    ...
    {if count($items)}
                  ...
                  {foreach from=$items item="item"}
                      ...
                      {if !empty($item->product->images_link)}
                          {foreach from=$item->product->images_link item="img"}
                              {if $img->img_main}
                                  <img src="{url module="corecatalog" file="`$img->img_filename`" type="image" preset="catalog_bin"}" alt="{$item->product->cat_name}"/>
            ...
            *}
                <li><a href="{url href="alias=order.html"}"><span>{$count|string_format:"%.0f"}</span> {lang code="productsintheamount.catalog.text"} <span>{$cost|string_format:"%.2f"}</span> {currency get="shortname"}</a></li>
    {else}
        <li>
            <a href="{url href="alias=order.html"}"><span>{$count|string_format:"%.0f"}</span> {lang code="productsintheamount.catalog.text"} <span>{$cost|string_format:"%.2f"}</span> {currency get="shortname"}</a>
        </li>
    {/if}
    </ul>
    {if count($items)}
