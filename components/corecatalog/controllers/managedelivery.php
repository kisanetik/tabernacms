 ...
 * @package Taberna
 */
class controller_corecatalog_managedelivery extends rad_controller
{
    ...
    function setActiveJS()
    {
        ...
        if($itemId){
            $item = new struct_corecatalog_delivery(array('rdl_id'=>$itemId));
            $item->load();
    ...
    /**
     * @return struct_corecatalog_delivery
     */
    private function _assignFromRequest($item=null)
    {
        $item = $item?$item:new struct_corecatalog_delivery();
        if(!$item->rdl_lang) {
