 ...
 *
 */
class controller_corecatalog_bin extends rad_controller
{
    ...
    function addToBin()
    {
        ...
        if($cat_id && ($count>0) ){
            ...
            if($ex_item){
                ...
                $item = new struct_corecatalog_bp( array('bp_catid'=>$cat_id,'bp_count'=>$count) );
    ...
    function calcBasket()
    {
        ...
        if(count($items))
            foreach ($items as $id) {
                // Count of goods
                $count += $id->bp_count;
                $convertedCost = model_corecatalog_currcalc::calcCours( $id->bp_cost, $id->bp_curid );
