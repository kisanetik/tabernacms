...
class controller_corecatalog_managecatalog extends rad_controller
{
    /**
     * Root parent id
     *
                ...
                case 'editform':
                    ...
                    if ($this->request('action_sub')){
                        if ($this->request('action_sub') == 'edit'){
                            $result = $this->edit();
                            if (!$result || ($this->request('returntorefferer') == '1')) {
                                $this->editForm();
                            }
                        }
                    } else {
    ...
    function addNode()
    {
        ...
        if ($node_id){
            ...
            $item = new struct_coremenus_tree();
    ...
    function applyEditNode()
    {
        if ($this->request('hash') == $this->hash()){
            $item = new struct_coremenus_tree($this->getAllRequest());
    ...
    private function _getProductTypesItems(&$items, $cat_id = NULL)
    {
        for($i = 0; $i<count($items); $i++){
            model_corecatalog_types::autoloadPlugins($items[$i]->vl_type_in);
    ...
    private function _getImageExtension($fileadr = null){
        $fileInfo = @getimagesize($fileadr);
        if (empty($fileInfo['mime'])) return null;
     ...
     *
     * @access private
     * @return array of struct_corecatalog_cat_files
     */
        ...
        if (!empty($_FILES[$data_name])){
            foreach($_FILES[$data_name]['name'] as $orig_name_id => $orig_name){
                if ((!$_FILES[$data_name]['error'][$orig_name_id]) and file_exists($_FILES[$data_name]['tmp_name'][$orig_name_id]) and ((int)$_FILES[$data_name]['size'][$orig_name_id])){
                    if ($cat_id){
                        $return[$orig_name_id] = new struct_corecatalog_cat_files(array('rcf_cat_id' => $cat_id, 'rcf_name' => $_FILES[$data_name]['name'][$orig_name_id]));
                    } else {
                        $return[$orig_name_id] = new struct_corecatalog_cat_files(array('rcf_name' => $_FILES[$data_name]['name'][$orig_name_id]));
                    }
        ...
        if (!empty($_FILES[$data_name])){
            foreach($_FILES[$data_name]['name'] as $orig_name_id => $orig_name){
                if ((!$_FILES[$data_name]['error'][$orig_name_id]) and file_exists($_FILES[$data_name]['tmp_name'][$orig_name_id]) and ((int)$_FILES[$data_name]['size'][$orig_name_id])){
                    ...
                    move_uploaded_file($_FILES[$data_name]['tmp_name'][$orig_name_id], CORECATALOG_IMG_PATH.$return[$orig_name_id]->img_filename);
                }
            }
        }
     ...
     * @return struct_corecatalog_catalog
     */
    private function _assignProductFromRequest($cat_id = NULL)
    {
        $product = new struct_corecatalog_catalog();
        ...
        if ($product->cat_ct_id){
            $product->type_link = rad_instances::get('model_coremenus_tree')->getItem($product->cat_ct_id);
            $model_ct = rad_instances::get('model_corecatalog_types');
                ...
                foreach($this->request('val_name') as $vl_name_id => $vl_value){
                    ...
                    if (is_array($vl_value)){
                        ...
                        foreach($vl_value as $vv_id => $vv_value){
                            $product->type_vl_link[$cnt]->vv_values[$i] = new struct_corecatalog_cat_val_values();
                        ...
                        $product->type_vl_link[$cnt]->vv_values[] = new struct_corecatalog_cat_val_values();
                        $product->type_vl_link[$cnt]->vv_values[0]->vv_name_id = (int)$vl_name_id;
        ...
        if ($this->request('product_tree')){
            ...
            foreach($this->request('product_tree') as $id => $tre_id){
                if ($cat_id){
                    $product->tree_link[] = new struct_corecatalog_cat_in_tree(array('cit_tre_id' => $tre_id, 'cit_cat_id' => $cat_id));
                } else {
                    $product->tree_link[] = new struct_corecatalog_cat_in_tree(array('cit_tre_id' => $tre_id));
                }
            }
        } else { //DEFAULT TREE PARENT PID
            $product->tree_link = array();
            if ($cat_id){
                $product->tree_link[] = new struct_corecatalog_cat_in_tree(array('cit_tre_id' => $this->_pid, 'cit_cat_id' => $cat_id));
            } else {
                $product->tree_link[] = new struct_corecatalog_cat_in_tree(array('cit_tre_id' => $this->_pid));
            }
        }
        ...
        //if del_img
        //Sets the default image default_image
        ...
        if ($this->_have_downloads){
            ...
            if ($this->request('del_file')){
                foreach($this->request('del_file') as $file_id => $on){
                    rad_instances::get('model_core_files')->deleteFilesByCat($file_id, true);
                }
                //foreach
            }
            //if del_img
            ...
            if (strlen(trim($this->request('producttags')))){
                ...
                if (count($tags_array)){
                    foreach($tags_array as $id_tag => $tag_id){
                        $product->tags[] = new struct_coreresource_tags_in_item(array('tii_item_id' => $product->cat_id, 'tii_tag_id' => $tag_id));
                    }
                }
            }
        }
    ...
    private function getProductCategoryId()
    {
        foreach ($this->product->tree_link as $obj) {
            if ($obj->cit_tre_id != $this->_pid) {
                return $obj->cit_tre_id;
            }
        }
        return false;
    }
    ...
    {
        $errors = array();
        if (empty($this->product->cat_name)) {
            $errors[] = $this->lang('enterproductname.catalog.message');
        }
        if ($this->product->cat_cost <= 0) {
            $errors[] = $this->lang('enterproductcost.catalog.message');
        }
        if (!$this->getProductCategoryId()) {
            $errors[] = $this->lang('enterproductcategory.catalog.error');
        }
        if (!empty($errors)) {
    ...
    function deleteProduct()
    {
        if ($this->request('hash') == $this->hash()){
            $cat_id = (int)$this->request('cat_id');
            if ($cat_id){
                $rows = rad_instances::get('model_corecatalog_catalog')->deleteProductById($cat_id);
                if ($rows){
    ...
    function deleteProductNoJS()
    {
        ...
        $cat_id = (int)$this->request('cat_id');
        rad_instances::get('model_corecatalog_catalog')->deleteProductById($cat_id);
        $parent_id = (int)$this->request('parent_id');
    ...
    function setActiveJS()
    {
        if ($this->request('hash') == $this->hash()){
            ...
            if ($cat_id){
                $r = rad_instances::get('model_corecatalog_catalog')->setActive($cat_id, $v);
    ...
    {
        $model = rad_instances::get('model_coresession_taberna');
            ...
            if (!empty($res)){
                ...
                if (isset($result->progress)){
                    if ((int)$result->progress>=100){
                        ...
                        $item = new struct_corecatalog_cat_3dimages(array(
                                                             'img_cat_id' => $catId,
                                                             'img_filename' => basename($fileName)
                                                        ));
                        rad_instances::get('model_corecatalog_3dimages')->insertItem($item);
                    }
    ...
    {
        $this->setVar('items_3dbin', rad_instances::get('model_corecatalog_3dimages')->setState('cat_id', $catId)->getItems());
    }
