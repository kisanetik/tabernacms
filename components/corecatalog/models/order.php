 ...
 * @package Taberna
 */
class model_corecatalog_order extends rad_model
{
    /**
     * Insert order to order table and inserts the order positions, without delete the original positions
     *
     * @param struct_corecatalog_orders $item
     * @return integer
     */
    function insertItem(struct_corecatalog_orders $item,$insertPositions = true)
    {
        ...
        if($insertPositions){
            ...
            if( count( $positions ) and $order_id ){
                ...
                foreach($positions as $id){
                    ...
                    $item = new struct_corecatalog_order_positions();
                    $item->orp_catid = $id->bp_catid;
                    $item->orp_orderid = $order_id;
                    $item->orp_count = $id->bp_count;
                    $item->orp_cost = model_corecatalog_currcalc::calcCours( $id->bp_cost, $id->bp_curid );
                    $item->orp_curid = model_corecatalog_currcalc::$_curcours->cur_id;
                    $item->orp_currency = model_corecatalog_currcalc::$_curcours->cur_ind;
                    /*
                    $item->orp_cost = $id->bp_cost;
                    $item->orp_curid = $id->bp_curid;
                    $item->orp_currency = model_corecatalog_currcalc::getCurrencyByID($id->bp_curid);
                    */
    ...
    function getItems($limit=NULL)
    {
        ...
        foreach( $this->queryAll( $q->toString().$limit, $q->getValues() ) as $key) {
            $order = new struct_corecatalog_orders($key);
     ...
     * @return struct_corecatalog_orders
     */
    function getItem($order_id=NULL, $with_positions=false)
    {
        if($order_id) {
            $result = new struct_corecatalog_orders( $this->query('SELECT * FROM '.RAD.'orders WHERE order_id=?', array((int)$order_id)) );
            ...
            if(count( $this->getStates() ) ){
                ...
                if($this->getState('select')=='count(*)') {
                    ...
                    if(!$this->getState('select',false)) {
                        return new struct_corecatalog_orders( $this->query( $q->toString().' LIMIT 1', $q->getValues() ) );
                    } else {
    ...
    function assignPositions(&$result, $orderId)
    {
        foreach( $this->queryAll('SELECT * FROM '.RAD.'order_positions where orp_orderid=?', array((int)$orderId)) as $id) {
            $result[] = new struct_corecatalog_order_positions($id);
        }
        return $result;
    }
    ...
    function updateItem(struct_corecatalog_orders $item)
    {
