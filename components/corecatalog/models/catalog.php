 ...
 * @package Taberna
 */
class model_corecatalog_catalog extends rad_model
{
     ...
     * @param string $limit
     * @return mixed array of struct_corecatalog_catalog | mixed array | NULL
     */
    ...
    function getItem($id=NULL)
    {
        ...
        if($id or $this->getStates()) {
            ...
            $q = $this->_getListQuery();
            $return = new struct_corecatalog_catalog($this->query( $q->toString().' LIMIT 1', $q->getValues() ));
            if($id) {
            ...
            if($this->getState('with_vv', false)) {
                ...
                $return->type_link = rad_instances::get('model_corecatalog_types')->getItem($return->cat_ct_id);
            }
     ...
     */
    function assignSpecial(struct_corecatalog_catalog $item)
    {
     ...
     * @param struct_corecatalog_catalog array $result
     * @param integer $ct_showing
        ...
        if(count($result)) {
            ...
            for($i=0;$i<count($result);$i++) {
                ...
                foreach($this->queryAll($q1->toString(), $q1->getValues()) as $id) {
                    $result[$i]->type_vl_link[] = new struct_corecatalog_cat_val_names($id);
                }
                if( count($result[$i]->type_vl_link) ) {
                    for($k=0;$k<count($result[$i]->type_vl_link);$k++ ) {
                        foreach( $this->queryAll('SELECT * FROM '.RAD.'cat_val_values WHERE vv_name_id='.$result[$i]->type_vl_link[$k]->vl_id.' AND vv_cat_id='.$result[$i]->cat_id.' ORDER BY vv_position') as $idv) {
                            $result[$i]->type_vl_link[$k]->vv_values[] = new struct_corecatalog_cat_val_values($idv);
                        }//foreach
                    }//for
                }
            }//for
    ...
    function insertItem(struct_corecatalog_catalog $item)
    {
     ...
     */
    function updateStruct(struct_corecatalog_catalog $item)
    {
    ...
    function updateItem(struct_corecatalog_catalog $item)
    {
        //update product here
        $rows = $this->updateStruct($item);
        //Update type values
        if(count($item->type_vl_link)) {
            $rows += rad_instances::get('model_corecatalog_types')->deleteTypeValuesByCatId($item->cat_id);
                ...
                if(count($tvl->vv_values)) {
                    foreach($tvl->vv_values as $vv) {
                        ...
                        if( strlen($vv->vv_value) or strlen($vv->vv_value2) ) {
                            ...
                            $rows += rad_instances::get('model_corecatalog_types')->insertTypeValues($vv);
                        }
                    }//foreach vv
                }
            }//foreach tvl
        ...
        */
        ...
        //IMAGES
        if( count($item->images_link) ) {
            $modelImage = rad_instances::get('model_core_image');
    ...
    function deleteProductById($id='')
    {
        if((int)$id){
            ...
            //DELETE COMMENTS
            $itemsComments = rad_instances::get('model_coreresource_comments')
                                ->setState('rcm_type', model_coreresource_comments::TYPE_PRODUCT)
                                ->setState('item_id', $id)
                                ->getItems();
     ...
     */
    function deleteItem(struct_corecatalog_catalog $item)
    {
    ...
    function getCatalogTreeEntries($cat_id)
    {
        ...
        if(count($rows))
            foreach($rows as $id)
                $return[] = new struct_corecatalog_cat_in_tree($id);
        return $return;
    }
     ...
     * @param struct_corecatalog_catalog $item
     */
    function assignDownloadFiles(struct_corecatalog_catalog &$item)
    {
        if($item and $item->cat_id) {
            $res = $this->queryAll('SELECT * FROM '.RAD.'cat_files WHERE rcf_cat_id=?', array($item->cat_id));
            if(!empty($res)) {
                foreach($res as $id) {
                    $item->download_files[] = new struct_corecatalog_cat_files($id);
                }
            }
        }
    }

    function assignComments(struct_corecatalog_catalog $item)
    {
        if($item and $item->cat_id) {
            $res = $this->queryAll('SELECT t.* FROM '.RAD.'comments c WHERE rcm_type=\'product\' AND rcm_item_id=?', array($item->cat_id));
            if(!empty($res)) {
                foreach($res as $id) {
                    $item->comments[] = new struct_coreresource_comments($id);
                }
            }
        }
    }
