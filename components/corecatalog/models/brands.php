 ...
 * @package Taberna
 */
class model_corecatalog_brands extends rad_model
{

    /**
     * Gets one struct_corecatalog_cat_brands by it ID
     * @param integer $id
     */
    function getItem($id=NULL)
    {
        ...
        if($id) {
            return new struct_corecatalog_cat_brands( $this->query('SELECT * FROM '.RAD.'cat_brands where rcb_id=?',array($id)) );
        } else {
            if(count( $this->getStates() ) ) {
                ...
                if($this->getState('select')=='count(*)'){
                    ...
                    if(!$this->getState('select',false)) {
                        return new struct_corecatalog_cat_brands( $this->query( $q->toString().' LIMIT 1', $q->getValues() ) );
                    } else {
    ...
    /**
     * Gets the array of struct_corecatalog_cat_brands by states
     * @return struct_corecatalog_cat_brands
     */
    function getItems($limit=NULL)
    {
        ...
        if($this->getState('select') != 'count(*)') {
            foreach( $this->queryAll( $q->toString().$limit, $q->getValues() ) as $key) {
                $result[] = new struct_corecatalog_cat_brands($key);
            }
        } else {
     ...
     * Insert one row
     */
    function insertItem(struct_corecatalog_cat_brands $item)
    {
     ...
     * update one row
     */
    function updateItem(struct_corecatalog_cat_brands $item)
    {
    ...
    /**
     * Delete one row
     * @param struct_corecatalog_cat_brands $item
     */
    function deleteItem(struct_corecatalog_cat_brands $item)
    {
