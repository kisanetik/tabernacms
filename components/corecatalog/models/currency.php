 ...
 * @package Taberna
 */
class model_corecatalog_currency extends rad_model
{
    function getItem($id=NULL)
    {
        ...
        return new struct_corecatalog_currency( $this->query( $q->toString().' LIMIT 1' ) );
    }
    ...
    function getItems()
    {
        ...
        foreach( $this->queryAll( $q->toString() ) as $key) {
            $result[] = new struct_corecatalog_currency($key);
        }
        return $result;
    }

    function insertItem(struct_corecatalog_currency $item)
    {
    ...
    function deleteItem(struct_corecatalog_currency $item)
    {
        return $this->delete_struct($item, RAD.'currency');;
    }

    function updateItem(struct_corecatalog_currency $item)
    {
