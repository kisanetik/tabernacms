 ...
 * @package RADCMS
 */
class model_corearticles_articles extends rad_model
{
    function getItem($id='')
    {
        $id=($id)?$id:$this->getState('id');
        if($id) {
            $result = new struct_corearticles_articles($this->query('SELECT * FROM '.RAD.'articles where art_id=?', array($id)));
    ...
    function getItems($limit = null)
    {
        ...
        if($this->getState('select')!='count(*)') {
            foreach( $this->queryAll( $q->toString().$limit, $q->getValues() ) as $key) {
                $result[] = new struct_corearticles_articles($key);
                if($this->getState('with_tree')) {
                    $result[count($result)]->tree = new struct_coremenus_tree($key);
                }
            }
        } else {
     ...
     * Insert one row
     */
    function insertItem(struct_corearticles_articles $item,$binded=false)
    {
     ...
     * update one row
     */
    function updateItem(struct_corearticles_articles $item,$binded=false)
    {
        //TAGS
        $modelTags = rad_instances::get('model_coreresource_tags')->setState('tag_type','articles');
    ...
    /**
     * Delete one row
     * @param struct_corearticles_articles $item
     */
    function deleteItem(struct_corearticles_articles $item)
    {
