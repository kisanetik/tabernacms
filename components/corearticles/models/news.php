<?php
class model_corearticles_news extends rad_model
{
    /**
     * Gets one struct_corearticles_news by it ID
     * @param integer $id
     */
    function getItem($id=NULL)
    {
        ...
        if($id){
            $return = new struct_corearticles_news( $this->query('SELECT * FROM '.RAD.'news where nw_id=?',array($id)) );
            if($this->getState('with_tags')) {
                $modelTags = rad_instances::get('model_coreresource_tags')->setState('tag_type','news');
            ...
            if(count( $this->getStates() ) ){
                ...
                if($this->getState('select')=='count(*)'){
                    ...
                    if(!$this->getState('select',false)){
                        return new struct_corearticles_news( $this->query( $q->toString().' LIMIT 1', $q->getValues() ) );
                    }else{
    ...
    /**
     * Gets the array of struct_corearticles_news by states
     * @return struct_corearticles_news
     */
    function getItems($limit=NULL)
    {
        ...
        if($this->getState('select')!='count(*)'){
            foreach( $this->queryAll( $q->toString().$limit, $q->getValues() ) as $key){
                $result[] = new struct_corearticles_news($key);
            }
        }else{
     ...
     * Insert one row
     */
    function insertItem(struct_corearticles_news $item){
     ...
     * update one row
     */
    function updateItem(struct_corearticles_news $item){
     ...
     */
    function deleteItem(struct_corearticles_news $item)
    {
        $modelTags = rad_instances::get('model_coreresource_tags')->setState('tag_type','news');
