 ...
 *
 */
class controller_corearticles_managearticles extends rad_controller
{
    /**
     * Root parent id
     *
    ...
    function setActive()
    {
        if($this->request('hash') == $this->hash()) {
            ...
            if($id) {
                $r = rad_instances::get('model_corearticles_articles')->setActive($id,$v);
  ...
  /**
    ...
    function showEditForm()
    {
        ...
        if($art_id) {//EDIT
            $modelArticles = rad_instances::get('model_corearticles_articles');
            if($this->_have_tags) {
            ...
            $this->setVar('item',new struct_corearticles_articles());
        }
    ...
    private function _deleteAllImg($img){
        foreach(glob(COREARTICLES_IMG_GLOB.'articles'.DS.$img) as $filename) @unlink($filename);
        @unlink(COREARTICLES_IMG_PATH.'articles'.DS.$img);
    }
        ...
        if($this->request('hash') == $this->hash()) {
            ...
            if(isset($_FILES['articles_image']) && count($_FILES['articles_image'])) {
                if((int)$_FILES['articles_image']['size']) {
                    ...
                    if (!file_exists(COREARTICLES_IMG_PATH.'articles')) {//create folder, if not exists
    ...
    function deleteNode()
    {
        if($this->request('hash') == $this->hash()) {
            ...
            if($node_id) {
                ...
                $articlesModel = rad_instances::get('model_corearticles_articles');
...
/**
    ...
    function delArticle()
    {
        if($this->request('hash') == $this->hash()) {
            ...
            if($article_id) {
                $rows = rad_instances::get('model_corearticles_articles')->deleteItemById($article_id);
                if($rows) {
    ...
    */
    function deleteArticleNoJS()
    {
        ...
        $art_id = (int)$this->request('id');
        rad_instances::get('model_corearticles_articles')->deleteItemById($art_id);
