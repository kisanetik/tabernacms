 ...
 *
 */
class controller_corearticles_managenews extends rad_controller
{
    /**
     * Root parent id
     *
    ...
    */
    ...
    function setActive()
    {
        if($this->request('hash') == $this->hash()) {
            ...
            if($item_id){
                $r = rad_instances::get('model_corearticles_news')->setActive($item_id,$v);
    ...
    function editFormNews()
    {
        ...
        if($node_id) {
            ...
            if($newsId) {
                $modelNews = rad_instances::get('model_corearticles_news');
                if($this->_have_tags) {
                ...
                $this->setVar('item', new struct_corearticles_news( array('nw_datenews'=>now(),'nw_datenews_from'=>now(),'nw_datenews_to'=>now()) ) );
            }
    ...
    private function _deleteAllImg($img){
        foreach(glob(COREARTICLES_IMG_GLOB.'news'.DS.$img) as $filename) @unlink($filename);
        @unlink(COREARTICLES_IMG_PATH.'news'.DS.$img);
    }
    ...
    function saveNews()
    {
        if($this->request('hash') == $this->hash()) {
            ...
            if($this->request('action_sub')=='edit'){
                ...
                $news_item = new struct_corearticles_news( $this->getAllRequest() );
            }
            ...
            if(count($_FILES['news_image'])){
              if( ((int)$_FILES['news_image']['size']) and (!$_FILES['news_image']['error']) ) {
                    ...
                    move_uploaded_file($_FILES['news_image']['tmp_name'], COREARTICLES_IMG_PATH.'news'.DS.$news_item->nw_img);
              }
            }
                ...
                if(strlen(trim($this->request('newstags')))) {
                    ...
                    if(count($tags_array)) {
                        foreach($tags_array as $id_tag=>$tag_id) {
                            $news_item->tags[] = new struct_coreresource_tags_in_item(array('tii_item_id'=>$news_item->nw_id, 'tii_tag_id'=>$tag_id));
                        }
                    }
                }
            }
    ...
    function showEditNode()
    {
        $node_id = (int)$this->request('node_id');
        if($node_id){
            $item = rad_instances::get('model_coremenus_tree')->getItem($node_id);
            $this->setVar('item', $item);
    ...
    function deleteNode()
    {
        if($this->request('hash') == $this->hash()) {
            ...
            if($node_id){
                ...
                $newsModel = rad_instances::get('model_corearticles_news');
    ...
    */
    function deleteNewsNoJS()
    {
        ...
        rad_instances::get('model_corearticles_news')->deleteItemById($news_id);
