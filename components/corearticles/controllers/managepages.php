 ...
 *
 */
class controller_corearticles_managepages extends rad_controller
{
    /**
     * Root parent id
     *
    ...
    */
    ...
    private function _deleteAllImg($img){
        foreach(glob(COREARTICLES_IMG_GLOB.'pages'.DS.$img) as $filename) @unlink($filename);
        @unlink(COREARTICLES_IMG_PATH.'pages'.DS.$img);
    }
    ...
    function savePage()
    {
        if($this->request('hash') == $this->hash()) {
            ...
            if($this->request('action_sub')=='edit'){
                ...
                $pages_item = new struct_corearticles_pages( $this->getAllRequest() );
            }
            ...
            if(count($_FILES['pages_image'])){
                if((int)$_FILES['pages_image']['size']){
                    ...
                    move_uploaded_file($_FILES['pages_image']['tmp_name'], COREARTICLES_IMG_PATH.'pages'.DS.$pages_item->pg_img);
                }//size
            }
                ...
                if(strlen(trim($this->request('pagetags')))) {
                    ...
                    if(count($tags_array)) {
                        foreach($tags_array as $id_tag=>$tag_id) {
                            $pages_item->tags[] = new struct_coreresource_tags_in_item(array('tii_item_id'=>$pages_item->pg_id, 'tii_tag_id'=>$tag_id));
                        }
                    }
                }
            }
    ...
    function deleteNode()
    {
        if($this->request('hash') == $this->hash()) {
            $node_id = (int)$this->request('node_id');
            if($node_id){
                //rad_instances::get('controller_core_managetree')->deleteItem($node_id);
                $model = rad_instances::get('model_coremenus_tree');
                ...
                $pagesModel = rad_instances::get('model_corearticles_pages');
...
/**
    ...
    function delPage()
    {
        if($this->request('hash') == $this->hash()) {
            ...
            if($page_id){
                $rows = rad_instances::get('model_corearticles_pages')->deleteItemById($page_id);
                if($rows){
    ...
    */
    function deletePageNoJS()
    {
        ...
        rad_instances::get('model_corearticles_pages')->deleteItemById($page_id);
