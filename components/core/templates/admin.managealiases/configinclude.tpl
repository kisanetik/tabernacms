    ...
    <div class="inn_components" style="background-color:#FFFFFF; margin-bottom: 15px;"><div>
        ...
        <div class="und_vkladki">
            <div id="TabsWrapperMain" class="wrap">
                <div id="configMain_tabcenter" class="lf_col tabcenter" style="{if empty($inc_item->params_presonal)} display:block;{else} display:none;{/if}width:100%;">
                                       ...
                                       {foreach from=$langs item=lng}
                                         {assign var=lngid value=$lng->lng_id}
                                           ({$lng->lng_code})<select name="param[{$item}][lang_{$lng->lng_id}]{if $items->_typeof($item) eq 'treenode.multiselect'}[]" multiple="multiple" style="height:100px;"{else}"{/if}>
                                               {radinclude module="core" file="admin.managealiases/tree_recursy.tpl" elements=$tree[$lngid] nbsp_count=0 ml=$lng->lng_id values=$values}
                                           </select><br />
                                       {/foreach}
                                    {else}
                                    <select name="param[{$item}]{if $items->_typeof($item) eq 'treenode.multiselect'}[]" multiple="multiple" style="height:100px;"{else}"{/if}>
                                        {radinclude module="core" file="admin.managealiases/tree_recursy.tpl" elements=$tree[$cur_lng_id] nbsp_count=0}
                                    </select>
                                    {/if}
                                ...
                                {elseif ($items->_typeof($item) eq 'bannerplaceselect') and isset($banner_places) and count($banner_places)}
                                    {if $items->_isMultilang($item)}
                                       {foreach from=$langs item=lng}
                                         {assign var=lngid value=$lng->lng_id}
                                           ({$lng->lng_code})<select name="param[{$item}][lang_{$lng->lng_id}]">
                                               {radinclude module="core" file="admin.managealiases/tree_recursy.tpl" elements=$banner_places[$lngid] nbsp_count=0 ml=$lng->lng_id values=$values}
                                           </select><br />
                                       {/foreach}
                                    {else}
                                    <select name="param[{$item}]">
                                        {radinclude module="core" file="admin.managealiases/tree_recursy.tpl" elements=$banner_places[$cur_lng_id] nbsp_count=0}
                                    </select>
                                    {/if}
                                {/if}
                            </td>
                       ...
                       {/if}
                        <tr>
                            ...
                            <td align="left" valign="top">
                                ...
                                {elseif ($items->_typeof($item) eq 'treenodeselect') or ($items->_typeof($item) eq 'treenode.multiselect')}{*add multiselect=attribute  multiple="multiple"*}
                                    {if $items->_isMultilang($item)}
                                       {foreach from=$langs item=lng}
                                           {assign var=lngid value=$lng->lng_id}
                                           ({$lng->lng_code})<select name="param[{$item}][lang_{$lng->lng_id}]{if $items->_typeof($item) eq 'treenode.multiselect'}[]" multiple="multiple" style="height:100px;"{else}"{/if}>
                                               {radinclude module="core" file="admin.managealiases/tree_recursy.tpl" elements=$tree[$lngid] nbsp_count=0 ml=$lng->lng_id values=$params_orig}
                                           </select><br />
                                       {/foreach}
                                    {else}
                                    <select name="param[{$item}]{if $items->_typeof($item) eq 'treenode.multiselect'}[]" multiple="multiple" style="height:100px;"{else}"{/if}>
                                        {radinclude module="core" file="admin.managealiases/tree_recursy.tpl" elements=$tree[$cur_lng_id] nbsp_count=0 values=$params_orig}
                                    </select>
                                    {/if}
                                {elseif ($items->_typeof($item) eq 'checkboxes')}
                                    {foreach from=$items->_get($item) item=ia key=kia}
                                ...
                                {elseif ($items->_typeof($item) eq 'bannerplaceselect') and isset($banner_places) and count($banner_places)}
                                    <select name="param[{$item}]">
                                        {radinclude module="core" file="admin.managealiases/tree_recursy.tpl" elements=$banner_places[$cur_lng_id] nbsp_count=0 values=$params_orig}
                                    </select>
                                {/if}
                            </td>
                        </tr>
