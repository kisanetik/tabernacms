 ...
 * @package RADCMS
 */
class model_core_settings extends rad_model
{
    function getItem($id=NULL)
    {
        $id = $id or $this->getState('id');
        if($id)
            return new struct_core_settings( $this->query('SELECT * FROM '.RAD.'settings WHERE recordid='.(int)$id) );
        else
            return NULL;
    }
    ...
    function getItems()
    {
        ...
        foreach( $this->queryAll( $q->toString() ) as $key){
            $result[] = new struct_core_settings($key);
        }
        return $result;
    }
    ...
    public function updateItem(struct_core_settings $struct)
    {
    ...
    function insertItem(struct_core_settings $struct)
    {
