 ...
 * @package RADCMS
 */
class controller_core_system extends rad_controller
{
    ...
    protected function _getPartnersLicense()
    {
        ...
        $licenseText = rad_instances::get('model_coresession_taberna')->getPartnersLicense($this->request('name'));
        die($licenseText);
    }
    ...
    protected function _acceptLicense()
    {
        if((bool)$this->request('accepted') and (int)$this->request('partner_id') and !empty($this->getCurrentUser()->u_id)) {
            ...
            if(rad_instances::get('model_coresession_taberna')->acceptLicense($pName)=='1') {
