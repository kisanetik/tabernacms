 ...
 * @pachage RADCMS
 */
class controller_coreothers_managevotes extends rad_controller
{
    private $_pid = 17;
    ...
    function editTree()
    {
        ...
        if($node_id) {
            ...
            $parents = array(new struct_coremenus_tree( array('tre_id'=>$this->_pid,'tre_name'=>$this->lang('rootnode.votes.text')) ));
    ...
    function deleteNode()
    {
        if($this->request('hash') == $this->hash()) {
            ...
            if($node_id) {
                ...
                $newsModel = rad_instances::get('model_coreothers_votes');
    ...
    function setActive()
    {
        if($this->request('hash') == $this->hash()) {
            ...
            if($item_id) {
                $r = rad_instances::get('model_coreothers_votes')->setActive($item_id,$v);
    ...
    function addItemForm()
    {
        ...
        if($cat_id) {
            ...
            $item = new struct_coreothers_votes();
            $this->setVar('item',$item);
            $this->setVar('ROOT_PID',$this->_pid);
        } else {
            $this->securityHoleAlert(__FILE__, __LINE__, $this->getClassName() );
     ...
     * @return struct_coreothers_votes
     */
    private function _assignRowFromRequest()
    {
        $item = new struct_coreothers_votes($this->getAllRequest());
    ...
    function saveRow()
    {
        if($this->request('hash') == $this->hash()) {
            ...
            if($item->vt_treid and strlen($item->vt_question) and $item->vt_id) {
                rad_instances::get('model_coreothers_votes')->updateItem($item);
            } else {
                $this->securityHoleAlert(__FILE__, __LINE__, $this->getClassName() );
            }
        } else {
    ...
    function deleteRow()
    {
        if($this->request('hash') == $this->hash()) {
            ...
            if($item_id) {
                $item_old = rad_instances::get('model_coreothers_votes')->getItem($item_id);
                $rows = rad_instances::get('model_coreothers_votes')->deleteItem($item_old);
   ...
   */
    ...
    function addEditAnswerForm()
    {
        ...
        if($item_id) {
            if($this->request('id')) {
                ...
                $item = new struct_coreothers_votes_questions();
    ...
    /**
     * @return struct_coreothers_votes_questions
     */
    private function _getAnswerFromRequest()
    {
        $item = new struct_coreothers_votes_questions($this->getAllRequest());
