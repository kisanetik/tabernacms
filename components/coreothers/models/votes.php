 ...
 * @package RADCMS
 */
class model_coreothers_votes extends rad_model
{
    function getItem($id=null)
    {
        $id=($id)?$id:$this->getState('id',$this->getState('vt_id'));
        if($id){
            $return = new struct_coreothers_votes($this->query('SELECT * FROM '.RAD.'votes where vt_id='.$id));
            return $return;
        }else{
    ...
    function getItems($limit = null)
    {
        ...
        foreach( $this->queryAll( $q->toString().$limit ) as $key){
            $result[] = new struct_coreothers_votes($key);
            $ids[] = $key['vt_id'];
        }
            ...
            foreach( $this->queryAll($sql) as $vtq){
                for($i=0;$i<count($result);$i++){
                    if($result[$i]->vt_id==$vtq['vtq_vtid']){
                        $result[$i]->vt_answers[] = new struct_coreothers_votes_questions($vtq);
                        break;
                    }
                }//for
            }//foreach
    ...
    /**
     * Insert one row
     * @param struct_coreothers_votes $item
     */
    function insertItem(struct_coreothers_votes $item)
    {
     ...
     * @param struct_coreothers_votes $item
     */
    function updateItem(struct_coreothers_votes $item)
    {
     ...
     * @param struct_coreothers_votes $item
     */
    function deleteItem(struct_coreothers_votes $item)
    {
