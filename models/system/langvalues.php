 ...
 * @package RADCMS
 */
class model_system_langvalues extends rad_model
{
    function getItems()
    {
        if($this->getState('type', 'translated')=='translated') {
            ...
            foreach( $this->queryAll($q->toString().$limit, $q->getValues()) as $id) {
                $result[] = new struct_langvalues($id);
            }
        } else {
    ...
    function getItemsGrouped($count = false)
    {
        ...
        foreach($this->queryAll($sql) as $row) {
            $result[] = new struct_langvalues($row);
        }
        return $result;
    }
    ...
    function getItem($id=null)
    {
        ...
        $table = new model_system_table(RAD.'langvalues');
    ...
    public function updateItem(struct_langvalues $struct)
    {
        return $this->update_struct($struct, RAD.'langvalues');
    }

    function insertItem(struct_langvalues $struct){
    ...
    function deleteItem(struct_langvalues $struct)
    {
