 ...
 */
final class model_system_params extends rad_model
{
    ...
    {
        $file = null;
        if(defined('SYSTEMMICROSPATH') and file_exists(SYSTEMMICROSPATH.$module.DS.'set'.DS.$fn.'.xml')) {
            $file = SYSTEMMICROSPATH.$module.DS.'set'.DS.$fn.'.xml';
        } else {
            if(defined('THEMESPATH') and file_exists(THEMESPATH.$module.DS.'set'.DS.$fn.'.xml')) {
                $file = THEMESPATH.$module.DS.'set'.DS.$fn.'.xml';
            } elseif( defined('MICROSPATH') ) {
                $file = MICROSPATH.$module.DS.'set'.DS.$fn.'.xml';
            }
        }
        return $file;
    }
    ...
    function setParamsForTemplate($params,$module,$fn)
    {
        ...
        if($file) {
            ...
            if( is_writable($file) ) {
            	return file_put_contents($file, $xmlObj->asXML() );
            } else {
            	return false;
            }
        }else{
    ...
    function setXMLStringForTemplate($xmlstring, $module, $fn)
    {
        ...
        if( $file and is_writable($file) ) {
            return !(file_put_contents($file, $xmlstring )===false);
        } else {
        	return false;
        }
    }
