...
/**
 * Catalog types
 * @deprecated 6 february 2009
 */
class model_catalog_pages extends rad_model
{
    function getItem($id='')
    {
        $id=($id)?$id:$this->getState('id');
        if($id){
        	$result = new struct_pages($this->query('SELECT * FROM '.RAD.'pages where pg_id=?', array($id)));
			if($this->getState('with_tags')) {
				$modelTags = rad_instances::get('model_resource_tags')->setState('tag_type','pages');
        	...
        	if($result) {
        		return new struct_pages($result);
        	} else {
        		return NULL;
        	}
        }else{
     ...
     * Insert one row
     */
    function insertItem(struct_pages $item,$binded=false)
    {
     ...
     * update one row
     */
    function updateItem(struct_pages $item,$binded=false)
    {
    	//TAGS
    	$modelTags = rad_instances::get('model_resource_tags')->setState('tag_type','pages');
     ...
     */
    function deleteItem(struct_pages $item)
    {
