 ...
 */
class model_catalog_3dimages extends rad_model
{
    /**
     * Saves item and check other fields
     * @param struct_cat_3dimages $item
     * @return Ambigous <number, number> - saved Rows count
     */
    public function insertItem(struct_cat_3dimages $item)
    {
    ...
    /**
     * Gets 3dImages
     * @return multitype:struct_cat_3dimages
     */
    public function getItems()
    {
        ...
        if(count($res)) {
            foreach($res as $id) {
                $result[] = new struct_cat_3dimages($id);
            }
        }
        return $result;
    }
     ...
     * @param array of struct_catalog $items
     */
     ...
     */
    public function assign3Dimage(struct_catalog $item)
    {
        $res = $this->queryAll('SELECT * FROM '.RAD.'cat_3dimages WHERE img_cat_id=?', array($item->cat_id));
        if(!empty($res)) {
            foreach($res as $id) {
                $item->models_3d[] = new struct_cat_3dimages($id);
            }
        }
    }
    ...
    {
        $item = new struct_cat_3dimages(array('img_id'=>$id));
        $item->load();
