 ...
 *
 */
class model_session_taberna extends rad_model
{
     ...
     * @param string $email
     * @param string $pass
     * @return struct_users|(int)error_code|false
     */
    ...
    {
        $settingsItem = new struct_settings(array(
    ...
    public function genere3DBin($params) 
    {
        ...
        $transactionId = $this->_getUrl($this->_threeDBinService, $params, array('file'=>array(
                'tmp_name'=>CATALOGORIGINALPATCH.$params['files'][0],
                'name'=>$params['files'][0])));
            ...
            if(count($params['files'])>1) {
                for($i=1;$i<count($params['files']);$i++) {
                    $transaction2Id = $this->_getUrl($this->_threeDBinService, $params, array('file'=>array(
                            'tmp_name'=>CATALOGORIGINALPATCH.$params['files'][$i],
                            'name'=>$params['files'][$i])));
                }
            }
    ...
    public function get3DBinFile($params)
    {
        ...
        if(!is_dir(dirname($destination))) {
            recursive_mkdir(dirname(str_replace($this->config('rootPath'), '', $destination)), 0777);
        }
