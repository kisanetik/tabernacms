...
{url file="jscss/components/radsocials.js" type="js"}
{if !empty($user->u_id)}
        ...
        {if $lang->lng_id neq $currentLangId and $lang->lng_active}
            <div class="lang-flag">
                <a href="{url href="alias=chlang&lang=`$lang->lng_code`&fromsite=true"}">
                    <img src="{const SITE_URL}image.php?w=36&h=36&m=lang&f={$lang->lng_img}" alt="{$lang->lng_name}" border="0" />
                </a>
            </div>
        {/if}
    {/foreach}
...
{/if}
    ...
    {url file="jscss/components/select/themes/jquery.ui.selectmenu.css" type="css" param="link"}
    {url file="jscss/components/select/select.css" type="css" param="link"}
    {url file="jscss/components/jquery/ui/jquery-ui.js" type="js"}
    {url file="jscss/components/jquery/ui/jquery.ui.selectmenu.js" type="js"}
    {url file="jscss/components/select/select.js" type="js"}
		...
		<fieldset>
			<select name="peopleB" id="peopleB">
                {foreach from=$currency item="cur"}
                    <option value="{$cur->cur_id}" class="css-avatar"{if $cur->cur_id eq $currentCurrency} selected="selected"{/if} style="background-image:url({const SITE_URL}img/des/default/currency/{$cur->cur_id}.png) ;">{$cur->cur_name|replace:' ':'&nbsp;'}{if $cur->cur_showcurs}&nbsp;({$cur->cur_cost}){/if}</option>
                {/foreach}
			</select>
		</fieldset>
	</div>
                ...
                {/if}
                    ...
                    {if !empty($params->is_facebook) or !empty($params->is_twitter)}
                        ...
                        {if $params->is_facebook}
                            <a href="javascript:RADSocials.socLogin('facebook');"><img src="{const SITE_URL}img/des/default/socials/FaceBook_24x24.png" border="0" align="middle"/> <span style="font-size:13px;">Facebook</span></a>
                            <br/>
                        {/if}
                        {if $params->is_twitter}                            
                            <a href="javascript:RADSocials.socLogin('twitter');"><img src="{const SITE_URL}img/des/default/socials/Twitter_24x24.png" border="0" align="middle"/> <span style="font-size:13px;">Twitter</span></a>
                        {/if}                            
                        </div>
                    {/if}
                </div>
