...
{url file="jscss/components/jquery/lightbox/jquery.lightbox-0.5.css" type="css" param="link"}
{url file="jscss/components/radcaptcha.js" type="js"}
{url file="jscss/components/jquery/lightbox/jquery.lightbox-0.5.pack.js" type="js"}
<script type="text/javascript">
            ...
            {if $model->img_main neq 0}
                ...
                <div class="image-3d-model" id="image-3d-model" style="margin-left: 15px">
                    <a href="#" onclick="product.image_3d_click();return false;">
                        <img src="{const SITE_URL}img/des/default/3d-bin-icon.png">&nbsp;{lang code="3dview.catalog.link" ucf=true}
                    </a>
                </div>
    ...
    {foreach from=$item->images_link item="img"}
        {if $img->img_main neq 0}
            <div class="pointer" style="float : left;">
                <img onclick="product.image_click()" src="{const SITE_URL}image.php?f={$img->img_filename}&w=155&h=300&m=catalog" class="goods_pic big" alt="{$item->cat_name|replace:'"':'&quot;'}" title="{$item->cat_name|replace:'"':'&quot;'}" alt="{$item->cat_name|replace:'"':'&quot;'}" />
                {$model3d}
            </div>
        {/if}
    {/foreach}  
{else} 
     <img src="{const SITE_URL}img/des/default/No_image_available.png"  width="155" height="300" />
{/if}
...
<div class="productpage">
    ...
    <div class="servfon goodsprice">
        ...
        <input type="button"{if !$item->cat_availability} disabled="disabled"{/if} class="btnblue tx wt fright buyBtn" value="{lang code="buy.catalog.title"}" {if $item->cat_availability}onclick="javascript:RADBIN.addToBin({$item->cat_id},this);"{/if}>
    </div>
    ...
    {if !empty($item->images_link)}
        <div class="tab {if $tab neq 'photo' and $tab neq 2}hide{/if}" id="tab_photo">
            ...
            {foreach from=$item->images_link item="img"}
                <div class="contfoto">
                    <div class="vertcentr">
                        <a class="lightbox" title="{$item->cat_name|@htmlspecialchars}" href="{const SITE_URL}image.php?f={$img->img_filename}&w=800&h=600&m=catalog">
                            <img src="{const SITE_URL}image.php?f={$img->img_filename}&w=140&h=140&m=catalog" title="{$item->cat_name|@htmlspecialchars}" alt="{$item->cat_name|replace:'"':'&quot;'}" />
                        </a>
                    </div>
                </div>
            {/foreach}
            ...
            <tr class="last">
                <td class="brdnn" colspan="2">&nbsp;
                    <div id="comment_preloader" style="display: none">
                        ...
                        <div class="canvas">
                            <img src="{const SITE_URL}jscss/components/mootree/mootree_loader.gif" width="16" height="16"/>
                        </div>
                    </div>
