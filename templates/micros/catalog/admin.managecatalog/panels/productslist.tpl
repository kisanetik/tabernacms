...
<tr>
    <td>
        <a id="active_cat_link_{$item->cat_id}_1" href="javascript:RADCatalogList.setActive(0,{$item->cat_id});" {if !$item->cat_active} style="display:none;"{/if}><img id="img_active_cat_{$item->cat_id}" src="{const SITE_URL}img/actions/activeround.gif" border="0" alt="{lang code='-turnoff' ucf=true|replace:'"':'&quot;'}" title="{lang code='-turnoff' ucf=true|replace:'"':'&quot;'}" /></a>
        <a id="active_cat_link_{$item->cat_id}_0" href="javascript:RADCatalogList.setActive(1,{$item->cat_id});" {if $item->cat_active} style="display:none;"{/if}><img id="img_active_cat_{$item->cat_id}" src="{const SITE_URL}img/actions/notactiveround.gif" border="0" alt="{lang code='-turnon' ucf=true|replace:'"':'&quot;'}" title="{lang code='-turnon' ucf=true|replace:'"':'&quot;'}" /></a>
    </td>
{if $params->showimages}
    <td>
        {if !empty($item->img_filename)}
        <a href="{const SITE_URL}image.php?f={$item->img_filename}&w={$params->originalmaxsize_x}&h={$params->originalmaxsize_y}&m=catalog" target="_blank"><img src="{const SITE_URL}image.php?f={$item->img_filename}&w={$params->thumbmaxsize_x}&h={$params->thumbmaxsize_y}&m=catalog" border="0" alt="{$item->cat_name|replace:'"':'&quot;'}" title="{$item->cat_name|replace:'"':'&quot;'}" /></a>
        {else}
            {lang code="-no"}
        {/if}
    </td>
{/if}
    ...
    <td nowrap="nowrap">
        ...
        <a href="{url href="alias=CATmanageCatalog&action=editform&cat_id=`$item->cat_id`"}">
            <img src="{const SITE_URL}img/backend/billiard_marker.png" alt="{lang code='-edit' ucf=true}" title="{lang code='-edit' ucf=true}" border="0" />
        </a>
        <a href="javascript:RADCatalogList.deleteRow({$item->cat_id})">
            <img src="{const SITE_URL}img/backend/icons/cross.png" alt="{lang code='-delete' ucf=true}" title="{lang code='-delete' ucf=true}" border="0" />
        </a>
    </td>
</tr>
{/foreach}
