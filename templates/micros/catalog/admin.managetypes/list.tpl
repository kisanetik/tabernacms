...
{else}
	 ...
	 {foreach from=$items item=item}
		<tr>
			...
			<td>
				<a href="javascript:RADCatTypesAction.addEditNewType('{$item->vl_tre_id}','{$item->vl_id}');"><img src="{const SITE_URL}img/backend/billiard_marker.png" alt="{lang code='-edit'}" title="{lang code='-edit'}" border="0" /></a><a href="javascript:RADCatTypesAction.delType('{$item->vl_id}','{$item->vl_tre_id}','{$item->vl_name}');"><img src="{const SITE_URL}img/backend/icons/cross.png" alt="{lang code='-delete' ucf=true}" title="{lang code='-delete' ucf=true}" border="0" /></a>
			</td>
		</tr>
	 {/foreach}
	 </table>
