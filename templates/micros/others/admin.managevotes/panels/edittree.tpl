...
<table cellpadding="3" cellspacing="0" border="0" width="100%">
    <tr>
        <td width="10%" nowrap="nowrap">{lang code='treename.banners.text'}</td>
        <td align="left"><input type="text" name="tre_name" id="tre_name" value="{$item->tre_name}" /></td>
    </tr>
    <tr>
        <td nowrap="nowrap">
            {lang code='treeposition.banners.text' }
        </td>
    ...
    <tr>
        <td>
            {lang code='parent.banners.text'}
        </td>
        <td align="left">
            <select id="tre_pid" name="tre_pid">
                {if isset($parents) and count($parents)}
                    {include file="$_CURRENT_LOAD_PATH/../menus/admin.managetree/tree_recursy.tpl" element=$parents selected=$item->tre_pid nbsp=0}
                {/if}
            </select>
        </td>
    </tr>
